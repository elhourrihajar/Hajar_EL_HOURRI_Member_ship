...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    System.out.println("Current token is ==> "+requestTokenHeader);
		...
		if (requestTokenHeader != null && requestTokenHeader.startsWith("Bearer ")) {
			...
			try {
				username = jwtTokenUtil.getLoginFromToken(jwtToken);
		...
		if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
			...
			User my_user = this.user_service.loadUserByUsername(username);

			// if token is valid configure Spring Security to manually set authentication
			if (jwtTokenUtil.validateToken(jwtToken, my_user)) {

				UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = new UsernamePasswordAuthenticationToken(
						my_user, null, my_user.getAuthorities());
